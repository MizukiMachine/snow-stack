...
type SceneAssetKey = CubeWorldAssetKey;
...
export class Renderer {
  ...
  private async loadCubeWorldAssetTemplates(generation: number): Promise<void> {
    const entries = Object.entries(CUBE_WORLD_ASSETS) as [SceneAssetKey, SceneAssetDefinition][];
    ...
    const loadSourceTemplate = (definition: SceneAssetDefinition): Promise<Group> => {
      ...
      const promise = this.gltfLoader.loadAsync(definition.path).then((gltf) => gltf.scene);
  ...
  private getSceneAssetMaterialColor(
    ...
    return definition.tint ?? source.color?.getHex() ?? 0xffffff;
  }
